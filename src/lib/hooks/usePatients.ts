...
export const usePatients = () => {
  ...
  const activePatients = usePatientStore(state => state.patients.filter(p => p.status === 'ACTIVE'));
  const inactivePatients = usePatientStore(state => state.patients.filter(p => p.status === 'INACTIVE'));
